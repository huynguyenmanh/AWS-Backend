...
Use a dedicated Cognito test account. The suite sends real requests and creates
and deletes DynamoDB and S3 test data.
...
$env:TODO_API_BASE_URL="https://YOUR_API_ID.execute-api.YOUR_AWS_REGION.amazonaws.com"
$env:TODO_API_ACCESS_TOKEN="YOUR_COGNITO_ACCESS_TOKEN_JWT"
pytest -v
```
...
export TODO_API_BASE_URL="https://YOUR_API_ID.execute-api.YOUR_AWS_REGION.amazonaws.com"
export TODO_API_ACCESS_TOKEN="YOUR_COGNITO_ACCESS_TOKEN_JWT"
pytest -v
```

The suite covers all documented route groups, real presigned S3 upload flows,
validation failures, version conflicts, workflow-reference conflicts, and
negative checks for the removed task-tag functionality.

### Destructive account-deletion test

The account-deletion test is skipped unless explicitly enabled. It permanently
deletes the test user's Cognito account, DynamoDB records, and S3 objects. Run
it last and only with a disposable account:

```powershell
$env:TODO_API_ENABLE_ACCOUNT_DELETE="1"
pytest -v -m destructive
```

After it succeeds, the account and access token cannot be reused.

## Existing tag data

Removing the feature from the code does not automatically migrate records that
were already stored. If the table previously contained `TAG#...` items or old
tasks containing `tagIds`, those values remain in DynamoDB until separately
cleaned up. The current backend no longer creates or updates them.
